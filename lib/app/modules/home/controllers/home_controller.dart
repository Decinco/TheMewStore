...
class HomeController extends GetxController {
  ...
  Future<void> fetchProducts() async {
    try {
      ...
      final prodList =
          (respProd as List).map((e) => ProductStock.fromJson(e)).toList();

      // 2) Ofertas: solo los campos que nos importan, para evitar nulls
      final respOff =
          await client.from('offer').select('offer_id, discount_percentage');
      ...
      if (prodList.isNotEmpty) {
        minPrice.value =
            prodList.map((p) => p.price).reduce((a, b) => a < b ? a : b);
        maxPrice.value =
            prodList.map((p) => p.price).reduce((a, b) => a > b ? a : b);
      } else {
      ...
      expansionOptions.value =
          prodList.map((p) => p.expansionId.toString()).toSet().toList();
